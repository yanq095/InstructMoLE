...
class FluxKontextPipeline(
    ...
    def prepare_latents(
        ...
        image_latents_list = []
        image_ids_list = []
        offset_w = 0  # 横向拼接，宽度偏移
        for idx, image in enumerate(images+conds):
        ...
        if len(images) > 0:
            ...
            image_ids = torch.cat(image_ids_list[:len(images)], dim=0)   
        else:
            image_latents = None
            image_ids = None   
        if len(conds) > 0: 
            vae_conds_len = [] 
            conds = image_latents_list[len(images):]
            for cond in conds:
                vae_conds_len.append(cond.shape[1])
            cond_latents = torch.cat(conds, dim=1)  
            cond_ids = torch.cat(image_ids_list[len(images):], dim=0)    
        else:
            cond_latents = None
            cond_ids = None
        ...
        return latents, image_latents, latent_ids, image_ids, cond_latents, cond_ids, vae_conds_len
    @property
    ...
    def __call__(
        ...
        for image in conds:
            if image is not None and not (isinstance(image, torch.Tensor) and image.size(1) == self.latent_channels):
                ...
                image = self.image_processor.resize(image, image_height, image_width)
                image = self.image_processor.preprocess(image, image_height, image_width)
            conds_list.append(image)  
        # 4. Prepare latent variables
        num_channels_latents = self.transformer.config.in_channels // 4
        latents, image_latents, latent_ids, image_ids, cond_latents, cond_ids, vae_conds_len = self.prepare_latents(
            images=image_list,
            conds=conds_list,
