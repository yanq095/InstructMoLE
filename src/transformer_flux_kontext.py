...
from src.attn_processor import AttnProcessor
from src.resampler import Resampler
from src.lora_moe import LoRAMoE
...
class FluxSingleTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        cond_rotary_emb: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
        sparse_layout=None,
...
@maybe_allow_in_graph
class CombinedTimestepGuidanceCondProjEmbeddings(nn.Module):
    ...
    def __init__(self, embedding_dim, pooled_projection_dim):
        ...
        self.cond_embedder = PixArtAlphaTextProjection(
            pooled_projection_dim, embedding_dim, act_fn="silu"
        )

    def set_proj_emb(self, emb):
        self.time_proj = emb.time_proj
        self.timestep_embedder = emb.timestep_embedder
        self.guidance_embedder = emb.guidance_embedder
    ...
    def forward(self, timestep, guidance, pooled_projection):
        ...
        pooled_projections = self.cond_embedder(pooled_projection)
...
class FluxTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        use_cond = cond_hidden_states is not None
        with enable_lora((self.norm1.linear,)):
...
class FluxTransformer2DModel(
    ...
    def __init__(
        ...
        self.condition_type_embedder = None
    ...
    def add_type_embedding(self, state_dict_path=None):
        if state_dict_path is not None:
            state_dict = torch.load(state_dict_path, map_location=self.device)
            self.condition_type_embedder = Resampler(
                dim=768,
                depth=2,
                dim_head=64,
                heads=12,
                num_queries=1,
                embedding_dim=4096,
                output_dim=self.pooled_projection_dim,
                ff_mult=4,
            )
            self.condition_type_embedder.load_state_dict(
    ...
    def forward(
        ...
        cond_type_ids: torch.Tensor = None,
        ...
        if self.condition_type_embedder is not None:
            cond_type_embs = self.condition_type_embedder(
                encoder_hidden_states, pooled_projections
            ).squeeze(1)
            LoRAMoE.set_type_embedding(cond_type_embs)
        else:
            LoRAMoE.set_type_embedding(pooled_projections)
        ...
        with enable_lora((self.x_embedder,)):
            hidden_states = self.x_embedder(hidden_states)
        if cond_hidden_states is not None:
            cond_hidden_states = self.x_embedder(cond_hidden_states)
            L_cond = cond_hidden_states.shape[1]
        else:
            L_cond = 0

        with enable_lora((self.context_embedder,)):
        ...
        if img_ids.ndim == 3:
            ...
            img_ids = img_ids[0]
        if cond_ids is not None and cond_ids.ndim == 3:
            logger.warning(
                "Passing `cond_ids` 3d torch.Tensor is deprecated."
                "Please remove the batch dimension and pass it as a 2d torch Tensor"
            )
            cond_ids = cond_ids[0]
        ...
        B, L_hidden, _ = hidden_states.shape
        L_txt = encoder_hidden_states.shape[1]
        sparse_layout = (vae_conds_len, L_txt, L_hidden, L_cond)
        if (
            joint_attention_kwargs is not None
