...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", type=str, default="/opt/tiger/efficient_ai/UniCombine/output/moe/gedit/")
    parser.add_argument("--language", type=str, default="en", choices=["all", "en", "cn"])
    ...
    for language in languages:
        ...
        group_scores_semantics, group_scores_quality, group_scores_overall, group_scores_semantics_intersection, group_scores_quality_intersection, group_scores_overall_intersection = analyze_scores(
            save_path_new, [model_name], language=language, file_ext=args.language
        )
        print("\nOverall:")
        for group_name in GROUPS:
        ...
        for group_name in GROUPS:
            result["overall"][group_name] = {
                "semantics": group_scores_semantics[model_name][group_name],
                "quality": group_scores_quality[model_name][group_name],
                "overall": group_scores_overall[model_name][group_name]
            }
        result["overall_avg"] = {
            "semantics": group_scores_semantics[model_name]['avg_semantics'],
            "quality": group_scores_quality[model_name]['avg_quality'],
            "overall": group_scores_overall[model_name]['avg_overall']
        }
        for group_name in GROUPS:
            result["intersection"][group_name] = {
                "semantics": group_scores_semantics_intersection[model_name][group_name],
                "quality": group_scores_quality_intersection[model_name][group_name],
                "overall": group_scores_overall_intersection[model_name][group_name]
            }
        result["intersection_avg"] = {
            "semantics": group_scores_semantics_intersection[model_name]['avg_semantics'],
            "quality": group_scores_quality_intersection[model_name]['avg_quality'],
            "overall": group_scores_overall_intersection[model_name]['avg_overall']
        }
        results_json.append(result)
    # 保存到 JSON 文件
    json_save_path = os.path.join(save_path_new, "scores.json")
    with open(json_save_path, "w", encoding="utf-8") as f:
        json.dump(results_json, f, ensure_ascii=False, indent=2)
    print(f"\n结果已保存到 {json_save_path}")